...
** TODO Misc requirements

TODO think this could be tidier and more clear

These are instructions you may need for a fresh install of emacs using this init file:

- Running the emacs from pop shop was cause errors with the enviroment being completely different. fixed this by downloading directly from emacs (TODO add link) and installing using the script here.
...
#+begin_src emacs-lisp
  (setq-default
   ...
   track-mouse nil                                  ; Ignore Mouse Movement During Key Sequences
...
Some defaults to make working on mac nicer.
...
#+begin_src emacs-lisp
  ...
  ;;    "Comment or uncomment the current line, but don't move point."
  ;;    (interactive)
  ;;    (let ((orig-pos (point)))
  ;;      (comment-line 1)
  ;;      (goto-char orig-pos)))
...
I am using the doom modeline. TODO get link
...
These packages make everything in the minibuffer easier.
...
TODO this should maybe be moved somewhere else as lsp does all this. but uses this ? so maybe move it to ls
** Git
*** magit
#+begin_src emacs-lisp
  (use-package magit)
...
TODO needs a big tidy up, see https://github.com/rememberYou/.emacs.d/blob/master/config.org#org-mode
