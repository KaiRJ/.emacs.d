...
- [[#programming][Programming]]
  - [[#languages][Languages]]
  - [[#packages][Packages]]
- [[#waiting-writing-packages][WAITING Writing packages]]
  - [[#defaults][Defaults]]
  - [[#flyspell][Flyspell]]
  - [[#ispell][Ispell]]
...
- For =lsp-mode= you will need to install all the language servers, eg. =sudo apt install clangd=. But emacs should prompt you to install them automatically anyway as you open files, if you run into issues just install them from the terminal.
  ...
  - I also had to install =nodejs= to fix an error
    #+begin_src bash
...
- For =tree-sitter= if you don't already have the grammars installed you will need to run ~M-x tree-sitter-langs-install-grammars~.

- On =COSMIC DE= I was finding issues with child frames with packages such as [[*Corfu][Corfu]], in order to solve this I had to build Emacs from source with the option ~--with-pgtk~ to build Emacs for "Pure GTK" which is needed for Wayland, now the default on Ubuntu. I got this solution from [[https://beamjourney.com/emacs-30-ubuntu-24-04/][here]].
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    ((select-enable-clipboard t) ;; Merge system's and Emacs' clipboard
     ...
     (tab-always-indent t)) ;; I just want tab to indent
...
I'm taking advantage of the built-in [[https://www.masteringemacs.org/article/which-function-mode][which-func-mode]] in order to put breadcrumbs in the modeline. Here I also define the layout of my modeline. Note, to set the position of the breadcrumbs use the ~misc-info~ segment. In the future I'd like to create a segment specifically for the breadcrumbs and just add it to the modeline instead of defining the entire thing here.
    ...
    (use-package which-func
      ...
      (doom-modeline-def-modeline 'main
        '(eldoc bar window-state workspace-name window-number modals matches follow
                buffer-info remote-host misc-info buffer-position word-count parrot
                selection-info)
        ' (compilation objed-state project-name persp-name battery grip irc
...
** all-the-icons
This provides icons for files, directories, etc. Might need to also run =M-x all-the-icons-install-fonts=,
...
The [[https://github.com/rranelli/auto-package-update.el][auto-package-update]] package automates the process of updating installed packages managed by package.el. Instead of requiring users to manually invoke =package-list-packages= and update each package.
...
* Org
This section sets up all my =Org-mode= based packages.

** org-mode
These are the settings for a general Org file.
...
#+begin_src emacs-lisp
  (use-package org
    ...
    :custom
    ;; theme source-blocks as they would in their native mode
    ...
    ;; for automatically setting the timestamps in files
    (time-stamp-active t)
    ...
    :config
    (defvar my/roam-directory
      "~/Documents/Org/RoamNodes"
      "My main Org Roam directory.")

    (defvar my/global-bibliography-file
      (expand-file-name "biblio.bib" my/roam-directory)
      "My main BibTex references file.")
    )
#+end_src

*** org-bullets
The [[https://github.com/sabof/org-bullets][org-bullets]] package simply shows org-mode bullets as UTF-8 characters.
...
#+end_src
*** org-fragtog
Org mode supports inline image previews of LaTeX fragments. These can be toggled with =C-c C-x C-l=. [[https://github.com/io12/org-fragtog?tab=readme-ov-file][org-fragtog]] automates this, so fragment previews are disabled for editing when your cursor steps onto them, and re-enabled when the cursor leaves.
...
** org-roam
[[https://www.orgroam.com/manual.html][Org-roam]] provides utilities for maintaining a digital slip-box in [[https://orgmode.org/][Org-mode]]. This setup is based mostly on the workflows outlined [[https://jethrokuan.github.io/org-roam-guide/][here]] and [[https://www.riccardopinosio.com/blog/posts/zotero_notes_article.html][here]] (Note, the latter provides a snippet of [[https://www.riccardopinosio.com/blog/posts/zotero_notes_article.html#opening-zotero-pdf-from-org-roam][Javascript]] that needs to be added to Zotero to correct the =file= field in the bibliography so it opens in Zotero from Emacs).
...
#+begin_src emacs-lisp
  (use-package org-roam
    ...
    :custom
    (org-roam-directory my/roam-directory)
    (org-roam-dailies-directory "dailies/")
    (org-roam-completion-everywhere t)

    ;; Add filetags to the node search
    ...
    (org-roam-capture-templates
     '(("m" "main" plain "%?"
        ...
        :unnarrowed t)
       ("i" "inbox" plain "%?"
        :target (file+head "inbox/${title}.org"
                           "#+created: %U\n#+last_modified: %U\n#+title: ${title}\n#+filetags: :inbox:draft:\n\n")
        :immediate-finish t
        :unnarrowed t)
       ("r" "reference" plain
        (file "~/Documents/Org/Templates/ref-template.org")
...
*** citar
[[https://github.com/emacs-citar/citar][citar]] makes browsing and inserting references in org-roam very easy.

Before setting up citar I defining a couple of variables used to set the icons in the citar references buffer:
...
I'm also using this function, copied from [[https://www.riccardopinosio.com/blog/posts/zotero_notes_article.html][this blog]], for opening Zotero files from citar:

#+begin_src elisp
  (defun riccardo/citar-file-trust-zotero (oldfun &rest args)
  "Leave Zotero-generated file paths alone, especially zotero://..."
  (if (boundp 'native-comp-deferred-compilation-enabled)
      (let ((native-comp-deferred-compilation-enabled nil))
        (cl-letf (((symbol-function 'file-exists-p) #'always)
                  ((symbol-function 'expand-file-name)
                   (lambda (first &rest _) first)))
          (apply oldfun args)))
    (cl-letf (((symbol-function 'file-exists-p) #'always)
              ((symbol-function 'expand-file-name)
               (lambda (first &rest _) first)))
...
#+begin_src elisp
  (use-package citar
    ...
    :custom
    (org-cite-global-bibliography '(my/global-bibliography-file))
    (org-cite-insert-processor 'citar)
    (org-cite-follow-processor 'citar)
    (org-cite-activate-processor 'citar)
    (citar-bibliography my/global-bibliography-file)

    ;; My default citar templete when browsing references
    (citar-templates
     '((main    . "${title:48}   ${author editor:30%etal}    ${date year issued:4}")
       (suffix  . "    ${=key= id:15}    ${=type=:12}    ${tags keywords:*}")
       (preview . "${author editor:%sn%etal} (${year issued date}) ${title}, ${journal journaltitle publisher container-title collection-title}.\n")
       (note    . "Notes on ${author editor:%etal}, ${title}")))

    :config
    ;; Add the advice around the functions
    (advice-add 'citar-file-open :around #'riccardo/citar-file-trust-zotero)
    (advice-add 'citar-file--find-files-in-dirs :around #'riccardo/citar-file-trust-zotero)
    ;; Change how citar opens pdf files
    (add-to-list 'citar-file-open-functions '("pdf" . citar-file-open-external))

    ;; Opens files in zotero
    (org-link-set-parameters "zotero"
                             :follow (lambda (zpath)
                                       (browse-url
                                        (format "zotero:%s" zpath))))
    ;; Set the icons of the citar elements
    ;; This needs to be here as the functions defined before rely on citar-indicator-create being defined
...
*** citar-org-roam
The [[https://github.com/emacs-citar/citar-org-roam][citar-org-roam]] package provides tighter Citar and Org-Roam integration, by creating org-roam notes for references.
...
#+begin_src elisp
  (use-package citar-org-roam
    ...
    :config
    (citar-org-roam-mode)
    (add-to-list 'citar-org-roam-template-fields '(:citar-url . ("url" "doi")))
...
*** org-roam-UI
[[https://github.com/org-roam/org-roam-ui][Org-Roam-UI]] is a frontend for exploring and interacting with org-roam notes. Use ~M-x org-roam-ui-mode~ to enable the global mode. It will start a web server on http://127.0.0.1:35901/ and connect to it via a WebSocket for real-time updates.
  ...
  (use-package org-agenda
    ...
    (org-agenda-files
     (mapcar 'abbreviate-file-name
             (split-string
              (shell-command-to-string "find ~/Documents/Org/Agenda -name \"*.org\"")
              "\n")))
    ...
    (org-agenda-prefix-format '((agenda . "  %-18c "))) ;; Shorten prefix to just file name
    (org-agenda-hide-tags-regexp ".*") ;; Remove catagoies at end of entries

    ;; Add time when mark something as done
    (org-log-done 'time)

    ;; Set default column view headings: Task Total-Time Time-Stamp
    (org-columns-default-format "%50ITEM(Task) %TIMESTAMP_IA")
...
[[https://github.com/alphapapa/org-super-agenda?tab=readme-ov-file#org-super-agenda][org-super-agenda]] lets me see all my TODOs alongside my agenda, I find this very useful for planning out my week.
...
#+begin_src emacs-lisp
  (use-package org-super-agenda
    ...
    :custom
    ;; Set the header title of this custom view
    (org-super-agenda-header '((t (:foreground "#51afef" :weight bold :height 1.2))))
     ...
     '(("z" "Super view"
        ((agenda ""
                 ((org-agenda-span 'week)
                  ...
                  (org-agenda-skip-function '(org-agenda-skip-entry-if 'todo 'done)) ;; Remove DONE entries
                  ))
                  ...
                  ((org-agenda-overriding-header "")
                   (org-super-agenda-groups
                    '(
                      ...
                      (:name "General"
                             :order 20
                             :and (:tag "general" :not (:todo "WAITING")))
                      (:name "Pulse Prominence"
                             :order 30
                             :and (:tag "prominence" :not (:todo "WAITING")))
                      (:name "Shells Simulation"
                             :order 40
                             :and (:tag "shells" :not (:todo "WAITING")))
                      (:name "Xe137 Tagging"
                             :order 45
                             :and (:tag "xe137" :not (:todo "WAITING")))
                      (:name "XLZD Sandbox"
                             :order 50
                             :and (:tag "xlzd" :not (:todo "WAITING")))
                      (:name "Xenon Activation"
                             :order 55
                             :and (:tag "activation" :not (:todo "WAITING")))
                      (:name "Thesis"
                             :order 60
                             :and (:tag "thesis" :not (:todo "WAITING")))
                      (:name "Emacs"
                             :order 70
                             :and (:tag "emacs" :not (:todo "WAITING")))
                      (:name "Waiting"
                             :order 99
                             :todo "WAITING")
                      ))
                   ;; Remove file/category prefix only in this view
                   (org-agenda-prefix-format
                    '((todo . "  %s")
                      (tags . "  %s")
                      (search . "  %s")
                      (agenda . "  %-12t %s")))
                   ;; Hide tags from agenda entries
                   (org-agenda-hide-tags-regexp ".*")
                   ))))))

    :config
    ;; To make available the custom agenda on startup
...
** Consult
[[https://github.com/minad/consult][Consult]] offers a suite of commands for efficient searching, previewing, and interacting with buffers, file contents, and more.
...
* Programming
** Languages
This section installs any additional programming modes that I use and configures them to my likely - mainly setting them up to use [[https://www.gnu.org/software/emacs/manual/html_mono/eglot.html][Eglot]] as my LSP client. I am using eglot as it is built into Emacs and is light weight. The main benefit of using Language Server Protocol (LSP) to configure the management of your programming languages is that LSP servers are also used by other text editors, increasing contributions to these packages.

Note that since Eglot is built into Emacs I just need to configure it for each language (hence the ~:ensure nil~).

*** C/C++
I have also added a =before-save-hook= that will run ~eglot-format-buffer~ only if there is a ~.clang-format~ file present in the =project-root= directory (this code is based on [[https://eklitzke.org/smarter-emacs-clang-format][blog]]).

#+begin_src elisp
  (defun my/eglot-format-buffer-if-clang-format ()
    "Format buffer with Eglot only if a .clang-format file exists in the project root."
    (interactive)
  ...
  (defun my/clang-format-buffer-smart-on-save ()
    "Add auto-save hook for my/eglot-format-buffer-if-clang-format."
    (add-hook 'before-save-hook #'my/eglot-format-buffer-if-clang-format nil t))

  (use-package eglot
    :ensure nil
    :hook
    (c-ts-mode . eglot-ensure)
    (c-ts-mode . my/clang-format-buffer-smart-on-save)
    (c++-ts-mode . eglot-ensure)
    (c++-ts-mode . my/clang-format-buffer-smart-on-save)
    ...
    :commands (eglot-ensure
               ...
               eglot-format-buffer)
    :config
    (add-to-list 'eglot-server-programs '((c++-ts-mode c-ts-mode) . ("clangd-20"
                                                                     "--clang-tidy"))))
#+end_src

*** GDScript
Here I am adding the [[https://github.com/godotengine/emacs-gdscript-mode][emacs-gdscript-mode]] and configuring it to be able to see my Godot executable and point at my local build of the Godot documentation.
...
#+begin_src emacs-lisp
  (use-package gdscript-mode
    ...
    :custom
    (gdscript-godot-executable "flatpak run org.godotengine.Godot")
    (gdscript-docs-local-path "/home/kairj/git/docs/godot-docs")
    (gdscript-gdformat-save-and-format t))  ;; Save all buffers and format them with gdformat anytime Godot executable is run.
#+end_src


** TODO Packages
This section contains all the packages I am using for programming.
...
**** git-gutter
[[https://github.com/emacsorphanage/git-gutter][git-gutter]] is a simple package that shows the git difs to the side of each line that there is a change.
...
**** git-gutter-fringe
I'm also using the [[https://github.com/emacsorphanage/git-gutter-fringe][git-gutter-fringe]] package to make this look similar to VScode (as per [[https://ianyepan.github.io/posts/emacs-git-gutter/][this]] demo).
...
*** TODO dap-mode
learn how to use this properly
...
#+end_src
*** Corfu
[[https://github.com/minad/corfu][Corfu]] enhances in-buffer completion by displaying a compact popup with current candidates, positioned either below or above the point. Candidates can be selected by navigating up or down. Corfu is also built into Emacs.
...
*** indent-bars
Currently using [[https://github.com/jdtsmith/indent-bars][indent-bars]] for my indentation highlight as its easy to use and supports treesitter and showing bars on blank lines:
...
*** TODO Parenthesis
*** rainbow-delimiters
[[https://github.com/Fanael/rainbow-delimiters][rainbow-delimiters]] is a small package that highlights delimiters according to their depth to make it easier to see what matches what.
...
* WAITING Writing packages
Update most of these when writing thesis.
